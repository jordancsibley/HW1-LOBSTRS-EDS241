...
I believe the control sites are a good counter factual for our treatment sites. From viewing the metadata (and working with this particular dataset a lot in undergrad), I know that all of the sites are relatively similar in habitat composition - the main difference being the level of protection. However, there is likely some room for bias due to some sites being closer to each other than others. There is a chance that these sites are more similar to each other just because of their physical distance. Additionally, there may be the bias due to the fact that lobsters are confined to the bounds of a site. The good news is, California Spiny lobsters do not travel far distances, and are known to remain in within a single crevice. However, once hatched, the larvae will often be transported by the currents until they settle to the bottom. So looking at population growth/decline within a LTER site over time might not be indicative of the reproduction of the adult lobsters. These ecological factors are important to consider when determining the level of bias between control and treatment sites. 
...
The results of this model are similar to that of Poisson model in that the results of the intercept and predictor intercept are both 22.73 and 1.24 respectively. Now we need to see if the assumptions of this model are met. 
